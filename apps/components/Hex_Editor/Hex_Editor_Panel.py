#!/usr/bin/env python3
"""
Hex Editor with Three Panel Display for IMG Factory - Complete implementation with DFF file header parsing
Panel 1: Shows the address offset (00000FF0:)
Panel 2: Shows the hex values (00 40 00 40 00 40 7D 72 70 7C 20 00 D5 D5 D7 B8)
Panel 3: Shows the ASCII representation (.@.@.@}rp| .....)
...
class HexEditorDialog(QDialog):
    """
    A complete hex editor dialog with three-panel display and DFF file header parsing
    Panel 1: Address offset (00000FF0:)
    Panel 2: Hex values (00 40 00 40 00 40 7D 72 70 7C 20 00 D5 D5 D7 B8)
    Panel 3: ASCII representation (.@.@.@}rp| .....)
    """
    ...
    def init_ui(self):
        ...
        # Left side - Three-panel Hex view
    ...
    def create_hex_view(self):
        """Create the three-panel hex view"""
        # Container widget
        container = QFrame()
        ...
        # Panel 1: Address offset
        self.address_panel = QTextEdit()
        self.address_panel.setReadOnly(True)
        self.address_panel.setStyleSheet("""
            QTextEdit {
        ...
        self.hex_panel.setReadOnly(True)
        ...
        # Panel 3: ASCII representation
        self.ascii_panel = QTextEdit()
        self.ascii_panel.setReadOnly(True)
        self.ascii_panel.setStyleSheet("""
            QTextEdit {
                font-family: 'Courier New', monospace;
                font-size: 10pt;
                background-color: #f0f0f0;
        ...
        # Create scroll areas for synchronized scrolling
        address_scroll = QScrollArea()
        address_scroll.setWidgetResizable(True)
        address_scroll.setWidget(self.address_panel)
        address_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        address_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        
        hex_scroll = QScrollArea()
        hex_scroll.setWidgetResizable(True)
        hex_scroll.setWidget(self.hex_panel)
        hex_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        ...
        ascii_scroll.setWidget(self.ascii_panel)
        ascii_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        ascii_scroll.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        
        # Synchronize vertical scrolling
        address_scroll.verticalScrollBar().valueChanged.connect(hex_scroll.verticalScrollBar().setValue)
        hex_scroll.verticalScrollBar().valueChanged.connect(address_scroll.verticalScrollBar().setValue)
        hex_scroll.verticalScrollBar().valueChanged.connect(ascii_scroll.verticalScrollBar().setValue)
        ascii_scroll.verticalScrollBar().valueChanged.connect(hex_scroll.verticalScrollBar().setValue)
        
        # Add scroll areas to the panel splitter
        panel_splitter.addWidget(address_scroll)
        panel_splitter.addWidget(hex_scroll)
        panel_splitter.addWidget(ascii_scroll)
        
        # Set sizes for the panels
        panel_splitter.setSizes([100, 400, 200])
    ...
    def display_hex_data(self):
        """Display the hex data in three panels"""
    ...
    def show_hex_context_menu(self, position):
        ...
        menu.exec(self.hex_panel.mapToGlobal(position))
    ...
    def goto_structure_offset(self):
        ...
        try:
            ...
            if selected_items:
                ...
                hex_text = self.hex_panel.toPlainText()
                ...
                if target_line >= 0:
                    ...
                    self.hex_panel.ensureCursorVisible()
