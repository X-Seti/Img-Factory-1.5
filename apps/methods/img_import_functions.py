...
def add_multiple_files_to_img(file_object, file_paths: List[str], main_window=None) -> Tuple[List, List]: #vers 2
...
def import_file(img_archive, file_path: str, entry_name: Optional[str] = None) -> bool: #vers 2
    ...
    try:
        ...
        # Use the add_entry method from the archive for proper metadata handling
        success = False
        if hasattr(img_archive, 'add_entry') and callable(img_archive.add_entry):
            try:
                ...
                if img_debugger:
                    img_debugger.error(f"Failed to add entry using add_entry: {str(e)}")
                # Fallback to add_entry_safe
                if img_debugger:
                    img_debugger.debug("Falling back to add_entry_safe")
                success = add_entry_safe(img_archive, entry_name, file_data)
        else:
            # If no add_entry method, use add_entry_safe
            if img_debugger:
                img_debugger.debug("No add_entry method, using add_entry_safe")
            success = add_entry_safe(img_archive, entry_name, file_data)
        if success:
            # Trigger a refresh to ensure metadata is visible in the UI
            if hasattr(img_archive, 'refresh_metadata_for_new_entries'):
                img_archive.refresh_metadata_for_new_entries()
            return True
        else:
            if img_debugger:
...
def import_multiple_files(img_archive, file_paths: List[str], entry_names: Optional[List[str]] = None) -> Tuple[List, List]: #vers 2
...
def refresh_after_import(main_window) -> None: #vers 2
    ...
    try:
        ...
        # Force a full metadata refresh for new entries
        if hasattr(main_window, 'current_img'):
            img_archive = main_window.current_img
            if hasattr(img_archive, 'refresh_metadata_for_new_entries'):
                img_archive.refresh_metadata_for_new_entries()
    except Exception as e:
...
def integrate_img_import_functions(main_window) -> bool: #vers 2
    ...
    try:
        ...
        if hasattr(main_window, 'log_message'):
            ...
            main_window.log_message("   • FIXED: Proper metadata population after import")
        return True
    except Exception as e:
        if hasattr(main_window, 'log_message'):
