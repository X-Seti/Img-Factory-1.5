...
class IMGFile: #vers 5
    ...
    def detect_version(self) -> IMGVersion: #vers 5
        """Detect IMG version and platform from file - FIXED"""
        try:
            ...
            if self.file_path.lower().endswith('.img'):
                try:
                    with open(self.file_path, 'rb') as f:
                        header = f.read(4)
                        img_debugger.debug(f"IMG header: {header} ({header.hex() if header else 'None'})")

                        if header == b'VER2':
                            self.version = IMGVersion.VERSION_2
                            img_debugger.debug("✅ V2 format detected")
                            return IMGVersion.VERSION_2
                        else:
                            # FIXED: Don't assume V1, check if DIR exists
                            dir_path = self.file_path[:-4] + '.dir'
                            if os.path.exists(dir_path):
                                self.version = IMGVersion.VERSION_1
                                img_debugger.debug("✅ V1 format detected (has DIR file)")
                                return IMGVersion.VERSION_1
                            else:
                                # No DIR file and no VER2 - unknown format
                                img_debugger.error(f"IMG file has no VER2 header and no DIR file")
                                self.version = IMGVersion.UNKNOWN
                                return IMGVersion.UNKNOWN
                except Exception as e:
                    img_debugger.error(f"Error reading IMG header: {e}")
                    pass
    ...
    def debug_img_header(file_path): #vers 1
        """Debug IMG file header"""
        try:
            with open(file_path, 'rb') as f:
                first_16_bytes = f.read(16)
                print(f"First 16 bytes: {first_16_bytes}")
                print(f"Hex: {first_16_bytes.hex()}")
                print(f"As text: {first_16_bytes.decode('ascii', errors='ignore')}")

                # Check if it starts with VER2
                if first_16_bytes.startswith(b'VER2'):
                    entry_count = struct.unpack('<I', first_16_bytes[4:8])[0]
                    print(f"V2 detected - Entry count: {entry_count}")
                else:
                    print("No VER2 signature found")
    ...
    def _load_version1(self) -> bool: #vers 2
            """Load Version 1 IMG (DIR/IMG pair) - WITH VALIDATION"""
            try:
                dir_path = self.file_path
                img_path = self.file_path[:-4] + '.img'
                ...
                # CRITICAL FIX: Validate file size first
                dir_size = os.path.getsize(dir_path)
                img_debugger.debug(f"DIR file size: {dir_size:,} bytes")

                # Sanity check: DIR files shouldn't be huge
                if dir_size > 1024 * 1024:  # 1MB max for directory
                    img_debugger.error(f"DIR file too large ({dir_size:,} bytes) - likely wrong format")
                    return False
                ...
                # Parse DIR entries (32 bytes each)
                entry_count = len(dir_data) // 32
                img_debugger.debug(f"Calculated entry count: {entry_count:,}")

                # CRITICAL FIX: Validate entry count is reasonable
                if entry_count > 100000:  # 100k max entries
                    img_debugger.error(f"Entry count too high ({entry_count:,}) - likely parsing wrong file type")
                    return False

                if entry_count < 1:
                    img_debugger.error("No valid entries found")
                    return False
                ...
                for i in range(entry_count):
                    ...
                    # VALIDATION: Check if entry data makes sense
                    if entry_name and len(entry_name) > 0:
                        # Basic filename validation
                        if len(entry_name) > 24 or any(ord(c) < 32 for c in entry_name if c):
                            img_debugger.warning(f"Suspicious filename: '{entry_name}' (entry {i})")
                            continue
                ...
                img_debugger.info(f"✅ V1 parsing complete: {len(self.entries)} valid entries")
                return True
    ...
    def debug_dir_file_structure(dir_path, max_entries=10): #vers 1
        """Debug DIR file structure to understand the format"""
        try:
            with open(dir_path, 'rb') as f:
                dir_data = f.read()

            print(f"DIR file size: {len(dir_data):,} bytes")
            print(f"Potential entries: {len(dir_data) // 32:,}")
            print(f"First 64 bytes: {dir_data[:64].hex()}")

            # Check first few entries
            for i in range(min(max_entries, len(dir_data) // 32)):
                offset = i * 32
                entry_data = dir_data[offset:offset+32]
                ...
                print(f"Entry {i}: offset={entry_offset}, size={entry_size}, name='{entry_name}'")
    ...
    def debug_img_parsing_issue(self, file_path): #vers 1
        """Debug the IMG parsing to find why it's showing 3M entries instead of 1209"""
        try:
            img_debugger.info(f"🐛 Debug parsing: {os.path.basename(file_path)}")

            # Check file size first
            file_size = os.path.getsize(file_path)
            img_debugger.info(f"📁 File size: {file_size:,} bytes ({file_size/1024:.1f} KB)")

            # Check file signature
            with open(file_path, 'rb') as f:
                first_4_bytes = f.read(4)
                img_debugger.info(f"🔍 First 4 bytes: {first_4_bytes} ({first_4_bytes.hex()})")

                # Check if it's V2 format
                f.seek(0)
                if first_4_bytes == b'VER2':
                    img_debugger.info("✅ V2 format detected")
                    entry_count_bytes = f.read(4)[4:8]  # Skip VER2, read next 4 bytes
                    if len(entry_count_bytes) == 4:
                        entry_count = struct.unpack('<I', entry_count_bytes)[0]
                        img_debugger.info(f"📊 V2 entry count: {entry_count:,}")
                    else:
                        img_debugger.error("❌ Could not read V2 entry count")
                else:
                    # Check V1 format
                    img_debugger.info("🔍 Checking V1 format...")
                    f.seek(0)
                    dir_data = f.read(min(file_size, 1024))  # Read first 1KB to check

                    if len(dir_data) >= 32:
                        # Calculate potential entry count
                        potential_entries = len(dir_data) // 32
                        img_debugger.info(f"📊 V1 potential entries from first 1KB: {potential_entries}")

                        # Check if full file would give 3M entries
                        full_potential = file_size // 32
                        img_debugger.info(f"📊 V1 full file potential entries: {full_potential:,}")

                        # This is likely the problem! ^^^
                        if full_potential > 1000000:
                            img_debugger.error(f"❌ FOUND ISSUE: File size {file_size:,} ÷ 32 = {full_potential:,} entries")
                            img_debugger.error("❌ This suggests the file is NOT a V1 DIR file!")

                            # Check if it might be a different format
                            f.seek(0)
                            sample = f.read(100)
                            img_debugger.info(f"🔍 File sample (first 100 bytes): {sample[:50].hex()}...")

                            return False

                        # Check first few entries for validity
                        f.seek(0)
                        for i in range(min(3, potential_entries)):
                            entry_data = f.read(32)
                            if len(entry_data) == 32:
                                offset, size = struct.unpack('<II', entry_data[:8])
                                name_bytes = entry_data[8:32].rstrip(b'\x00')

                                try:
                                    name = name_bytes.decode('ascii', errors='ignore')
                                    img_debugger.info(f"📄 Entry {i}: offset={offset}, size={size}, name='{name}'")
                                except:
                                    img_debugger.error(f"❌ Entry {i}: Could not decode name")
                    else:
                        img_debugger.error("❌ File too small for V1 format")

        except Exception as e:
            img_debugger.error(f"❌ Debug parsing failed: {e}")
    ...
    def _load_version2(self) -> bool: #vers 2
        """Load Version 2 IMG (single file) - FIXED FORMAT"""
        try:
            with open(self.file_path, 'rb') as f:
                # Read header
                header = f.read(4)
                if header != b'VER2':  # Use bytes literal
                    return False

                # Read entry count
                entry_count = struct.unpack('<I', f.read(4))[0]
                img_debugger.debug(f"V2 IMG entry count: {entry_count}")

                # VALIDATION: Reasonable entry count check
                if entry_count > 100000 or entry_count < 0:
                    img_debugger.error(f"Invalid entry count: {entry_count}")
                    return False

                # Read entries - CORRECT V2 FORMAT!
                self.entries = []
                for i in range(entry_count):
                    entry_data = f.read(32)
                    if len(entry_data) < 32:
                    ...
                    # CORRECT V2 parsing: offset(4), streaming_size(2), size(2), name(24)
                    offset_sectors = struct.unpack('<I', entry_data[:4])[0]
                    streaming_size = struct.unpack('<H', entry_data[4:6])[0]
                    size_sectors = struct.unpack('<H', entry_data[6:8])[0]
                    name_bytes = entry_data[8:32]

                    # Convert values - FIXED
                    actual_offset = offset_sectors * SECTOR_SIZE

                    # Use size_sectors if available, otherwise streaming_size
                    if size_sectors > 0:
                        actual_size = size_sectors * SECTOR_SIZE
                    else:
                        actual_size = streaming_size * SECTOR_SIZE
                ...
                img_debugger.success(f"V2 parsing complete: {len(self.entries)} valid entries")
                return True

        except Exception as e:
            img_debugger.error(f"Error opening Version 2 IMG: {e}")
            return False
