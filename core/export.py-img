...
def _start_export_with_progress(main_window, entries, export_folder, export_options): #vers 2
    """Start export with progress dialog - FIXED threading issues"""
    try:
        ...
        def export_finished(success, message, stats):
            try:
                # CRITICAL: Close progress dialog first
                if progress_dialog:
                    progress_dialog.close()
                    progress_dialog.deleteLater()
                
                # CRITICAL: Ensure thread is properly stopped
