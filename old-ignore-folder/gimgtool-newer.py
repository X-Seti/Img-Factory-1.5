...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s --extract models.img ./extracted/
  %(prog)s --create ver2 custom.img ./my_models/
  %(prog)s --add models.img car1.dff car2.dff texture.txd
  %(prog)s --del models.img old_car.dff unused.txd
  %(prog)s --rename models.img oldname.dff newname.dff
  %(prog)s --list textures.img "*.txd"
  %(prog)s --rebuild models.img --dry-run
        """
    )
    ...
    command_group.add_argument('--extract', '-x', action='store_true',
                              help='Extract files from IMG')
    command_group.add_argument('--create', '-c', metavar='FORMAT',
                              choices=['ver1', 'ver2'],
                              help='Create IMG from directory (ver1 or ver2)')
    command_group.add_argument('--add', '-a', action='store_true',
                              help='Add files to IMG')
    command_group.add_argument('--del', '-d', action='store_true',
                              help='Remove files from IMG')
    command_group.add_argument('--rename', '-r', action='store_true',
                              help='Rename file in IMG')
    command_group.add_argument('--rebuild', '-R', action='store_true',
                              help='Rebuild/optimize IMG')
    command_group.add_argument('--list', '-l', action='store_true',
                              help='List IMG contents')
    command_group.add_argument('--info', '-i', action='store_true',
                              help='Show IMG information')

    # Positional arguments
    parser.add_argument('img_file', help='IMG file to work with')
    parser.add_argument('args', nargs='*', help='Additional arguments based on command')
    ...
    with IMGTool() as tool:
        try:
            if args.extract:
                success = tool.extract_img(args.img_file, output_dir, dry_run=args.dry_run)
            elif args.create:
                success = tool.create_img(format_type, args.img_file, input_dir, dry_run=args.dry_run)
            elif args.add:
                success = tool.add_multiple_to_img(args.img_file, files, dry_run=args.dry_run)
            elif args.delete:
                success = tool.remove_multiple_from_img(args.img_file, files, dry_run=args.dry_run)
            elif args.rename:
                success = tool.rename_in_img(args.img_file, old_name, new_name, dry_run=args.dry_run)
            elif args.rebuild:
                success = tool.rebuild_img(args.img_file, dry_run=args.dry_run)
            elif args.list:
                success = tool.list_img_filtered(args.img_file, filter_pattern)
            elif args.info:
                success = tool.info_img(args.img_file)
            else:
                print(f"No command specified", file=sys.stderr)
                return 2
...
# Updated method signatures to support dry_run
class IMGTool:
    # ... existing methods ...

    def extract_img(self, img_file: str, output_dir: str, dry_run: bool = False) -> bool:
        """Extract all files from IMG archive"""
        if not os.path.isfile(img_file):
            print(f"Error: IMG file not found: {img_file}", file=sys.stderr)
            return False
        ...
        if dry_run:
            print(f"[DRY RUN] Would extract files to: {output_dir}")
        ...
        if dry_run:
            print(f"[DRY RUN] Would extract {entry_count} files from {img_file}")
        else:
            print(f"Extracting {entry_count} files from {img_file}")
        ...
        with open(img_file, 'rb') as f:
            for i in range(entry_count):
                ...
                try:
                    ...
                    if dry_run:
                        print(f"[DRY RUN] Would extract: {filename} (size: {file_size} bytes)")
                    else:
                        print(f"Extracting: {filename} (size: {file_size} bytes)")
        ...
        if dry_run:
            print("[DRY RUN] Extraction simulation complete!")
        else:
            print("Extraction complete!")
        return True

    def add_multiple_to_img(self, img_file: str, files_to_add: List[str], dry_run: bool = False) -> bool:
        """Add multiple files to existing IMG"""
        if not files_to_add:
            print("Error: No files specified to add", file=sys.stderr)
            return False
        ...
        if dry_run:
            print(f"[DRY RUN] Would add files to {img_file} (format: {format_type})")
            for file_path in files_to_add:
                if os.path.isfile(file_path):
                    basename = os.path.basename(file_path)
                    size = os.path.getsize(file_path)
                    print(f"[DRY RUN] Would add: {basename} ({size} bytes)")
                else:
                    print(f"[DRY RUN] Warning: File not found: {file_path}")
            return True
    ...
    def rebuild_img(self, img_file: str, dry_run: bool = False) -> bool:
        """Rebuild/optimize IMG file"""
        # Detect format
        format_type = self.detect_img_format(img_file)
        if format_type == "INVALID":
        ...
        if dry_run:
            print(f"[DRY RUN] Would rebuild {img_file} (format: {format_type})")

            # Get current stats
            old_size = os.path.getsize(img_file)
            entry_count = self.get_entry_count(img_file, format_type)

            print(f"[DRY RUN] Current size: {old_size} bytes")
            print(f"[DRY RUN] File count: {entry_count}")
            print(f"[DRY RUN] Would create backup: {img_file}.backup.{int(time.time())}")
            print(f"[DRY RUN] Rebuild simulation complete!")
            return True
